package competitiveProgramming.geekforgeeks.phase1.mustdoquestions.recursion;
...
Empty
 */
public class CombinationSumPart2 { // duplicates in the input
    public static void main(String[] args) {
        int[] arr = new int[]{10, 1, 2, 7, 6, 5, 1};
    ...
    private static void backtrack(List<Integer> temp, List<List<Integer>> result, int[] arr, int val, int sum, int start) {
        ...
        for (int i = start; i < arr.length; i++) {
            ...
            temp.add(arr[i]);
            backtrack(temp, result, arr, val, sum + arr[i], i + 1);
